$(function(){
	...
	$("a#clear").click(function(){
		...
		$('input,textarea').removeClass('not_valid');
	});
	...
	$("input#email").blur(function(){
		if(validateInput('email')){
			if(!validateEmail()){
				...
				$("input#email").addClass('not_valid');
			}
		}else{
	...
	$("input#phone").blur(function(){
		if(validateInput('phone')){
			if(!validatePhone()){
				...
				$("input#phone").addClass('not_valid');
			}
		}else{
	...
	$("textarea#message").blur(function(){
		if(validateTextArea('message')){
			if(!validateMessage()){
				...
				$("textarea#message").addClass('not_valid');
			}
		}else{
	...
	$("input#name").keydown(function(){
		if(validateInput('name')){
			$("label#name_error").fadeOut(0);
			$("input#name").removeClass('not_valid');
		}
		if(validateName()){
			$("label#name_error2").fadeOut(0);
			$("input#name").removeClass('not_valid');
	...
	$("input#email").keydown(function(){
		if(validateInput('email')){
			$("label#email_error").fadeOut(0);
			$("input#email").removeClass('not_valid');
		}
		if(validateEmail()){
			$("label#email_error2").fadeOut(0);
			$("input#email").removeClass('not_valid');
	...
	$("input#phone").keydown(function(){
		if(validateInput('phone')){
			$("label#phone_error").fadeOut(0);
			$("input#phone").removeClass('not_valid');
		}
		if(validatePhone()){
			$("label#phone_error2").fadeOut(0);
			$("input#phone").removeClass('not_valid');
	...
	$("textarea#message").keydown(function(){
		if(validateTextArea('message')){
			$("label#message_error").fadeOut(0);
			$("textarea#message").removeClass('not_valid');
		}
		if(validateMessage()){
			$("label#message_error2").fadeOut(0);
			$("textarea#message").removeClass('not_valid');
			...
			success: function(){
				...
				$('#contact_form').html("<div class='download-box'>Contact form submitted!</div>")
					...
					.fadeIn(1500, function(){
						$('#contact_form').append("<br><br><a id='back' onclick='window.location.reload(); return false;' class='button'><span>back</span></a>");
						$('.button').hover(function(){$(this).stop().animate({color:'#bbc5a8'},300)},function(){$(this).stop().animate({color:'#7eb900'})})
				});
			}
		});				
		return false;
