...
$(function(){
	...
	if($('#pro_contact-form').length||$('#contact-form-1').length)$.include('js/forms.html');
	
	
	if($('.pro_tweet').length)$.include('js/jquery.tweet.html');
	if($('.lightbox-image').length)$.include('js/jquery.prettyPhoto.html');	
	if($('.pro_kwicks').length)$.include('js/kwicks-1.5.1.pack.html');
	if($('#pro_counter').length)$.include('js/jquery.countdown.min.html');
	if($('.fixedtip').length||$('.clicktip').length||$('.normaltip').length)$.include('js/jquery.atooltip.pack.html')
// Code
